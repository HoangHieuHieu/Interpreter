...
(define class_method_closure
  (lambda (stmt classname)
    (list (get-func-body stmt) (cons 'this (get-func-formal-params stmt)) java_func_state  classname)))

(define java_func_state
  (lambda (state class-name l1 l2)
    (cond
      [(null? state) state]
      [(null? (name-list state)) (java_func_state (prev-frame state) (class-name) '() '())]
      [(eq? (car (name-list state)) class-name) (list (append l1 (name-list state)) (append l2 (val-list state)) (prev-frame state))]
      [else (java_func_state (cdr-state state) class-name (cons (car (name-list state)) l1) (cons (car (val-list state) l2)))])))

; (define class_method_exe
;   (lambda (funcall state throw)
;     (let ((method-closure ...)
;           (f ...)
;           (global (f state))
;           (formal-p
;           (f-state-1 (create-bindings formal-params actual-params (add-frame global)))
;           (f-state (add 'this object-closure f-state-1))
;           (compile-type classname)
...
(define class-closure
  (lambda (stmt)
    (let ((super (if (null? (super-class stmt))
      ...
      (cons super (interpret_class_body body '(()()()()) (class-name stmt))))))
...
(define interpret_class_body
   (lambda (body closure classname)
     (let ((field-names car)
       ...
       (cond
         [(null? body) closure]
         [(list? (operator body)) (interpret_class_body (car body) (interpret_class_body (cdr body) closure classname) classname)]
         ...
         [(eq? (operator body) 'function)(list (field-names closure) (field-inits closure) (cons (get-func-name body) (method-name closure)) (cons (class_method_closure body classname) (method-closures closure)))]
         [(eq? (operator body) 'static-function) (list (field-names closure) (field-inits closure) (cons (get-func-name body) (method-name closure)) (cons (class_method_closure body classname) (method-closures closure)))]))))

(define class_interpreter
  (lambda (stmt state)
    (cond
      [(null? stmt) state]
      [(list? (operator stmt)) (class_interpreter (car stmt) (class_interpreter (cdr stmt) state))]
      [(eq? (operator stmt) 'class) (add (class-name stmt) (class-closure stmt) state)]
